...
export function Benefits() {
  const iconColors = ['#000000', '#000000', '#000000', '#000000'];
      ...
      <div className="max-w-7xl mx-auto">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          <motion.div variants={slideInLeft} initial="hidden" whileInView="visible" viewport={{ once: true }} className="space-y-8">
            ...
            <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black tracking-tight" style={{ letterSpacing: "-0.03em", color: '#F9FAFB' }} id="beneficios-heading">Beneficios</h2>
                  ...
                  <motion.div key={idx} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.08, duration: 0.5 }} whileHover={{ y: -15, scale: 1.05, boxShadow: '0 20px 40px rgba(0, 102, 204, 0.2)' }} className="group">
                    <Card className="p-8 rounded-3xl cursor-pointer shadow-lg hover:shadow-2xl transition-shadow focus-visible:outline-2 focus-visible:outline-offset-2" style={{ backgroundColor: '#FFFFFF', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.3)' }}>
                      <CardBody className="flex flex-col items-center text-center">
                        <motion.div whileHover={{ rotate: 15, scale: 1.15 }} transition={{ type: 'spring', stiffness: 300 }} className="w-16 h-16 rounded-2xl flex items-center justify-center mb-6 shimmer-icon" style={{ backgroundColor: '#F3F4F6' }} data-icon-index={idx}>
                          <IconComponent size={40} className={`transition-colors ${item.hoverColor} icon-shimmer`} style={{ color: '#000000' }} />
                        </motion.div>
              ...
              {BENEFITS.slice(2, 4).map((item, idx) => {
                ...
                return (
                  <motion.div key={idx + 2} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: (idx + 2) * 0.08, duration: 0.5 }} whileHover={{ y: -15, scale: 1.05 }} className="group">
                    <Card className="p-8 rounded-3xl cursor-pointer shadow-lg hover:shadow-2xl transition-shadow focus-visible:outline-2 focus-visible:outline-offset-2" style={{ backgroundColor: '#FFFFFF', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.3)' }}>
                      <CardBody className="flex flex-col items-center text-center">
                        <motion.div whileHover={{ rotate: 15, scale: 1.15 }} transition={{ type: 'spring', stiffness: 300 }} className="w-16 h-16 rounded-2xl flex items-center justify-center mb-6 shimmer-icon" style={{ backgroundColor: '#F3F4F6' }} data-icon-index={idx + 2}>
                          <IconComponent size={40} className={`transition-colors ${item.hoverColor} icon-shimmer`} style={{ color: '#000000' }} />
                        </motion.div>
                        <h3 className="text-xl sm:text-2xl font-bold mb-3 leading-snug" style={{ color: '#1F2937' }}>{item.title}</h3>
                        <p className="text-base leading-relaxed" style={{ color: "#4B5563" }}>{item.desc}</p>
